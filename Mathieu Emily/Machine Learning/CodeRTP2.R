...
plot(data.test$x1,data.test$x2,col=1+class.test)
points(x,(3+cos(4*pi*x))/8+(x^2)/2,type="l",ylim=c(0,1),lty=6,col=4,lwd=4)
...
#pred.grid.deg2 <- predict(mod,newdata=data.grid,type="response") > 0.3
...
#contour(ss,ss,matrix(as.numeric(pred.grid.deg2),ncol=length(ss)),nlevels=1,add=TRUE,drawlabels=FALSE,col=3,lwd=1.5)
...
points(data.test$x1[1:20],data.test$x2[1:20],col=2+2*(data.test$class[1:20]),pch=19)
points(data.test$x1[21:40],data.test$x2[21:40],col=2+2*(data.test$class[1:20]),pch=19)
