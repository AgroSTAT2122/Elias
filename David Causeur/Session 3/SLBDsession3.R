...
pig = read.table("SLBDscanner.txt")
...
plot(1:137,mod$beta[,1],pch=16)

#Quand lambda est petit, on a une variance très très forte (biais négligé) 
#à l'inverse, quand lambda est grand, variance très faible mais biais très fort
...
#Deux choix, soit MSEP minimal, soit MSEP minimal + écart type
...
#Quand lambda était trop grand, ce n'est plus la peine de mettre x dans le modèle
...
#Compromis est entre 30 et 20 variables
...
#scale sur transposé de x : moyenne à 0 et écart type à 1
#Il faut retransposer pour pouvoir ravoir les bonnes dimensions
...
segs = groupdata2::fold(dta,k=10,cat_col="Localisation")$".folds"
